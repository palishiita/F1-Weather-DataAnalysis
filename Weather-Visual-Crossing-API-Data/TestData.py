...
file_path = r'C:\Users\ishii\Documents\Formula-1-And-Weather-Data-Engineering-Project\Weather-Visual-Crossing-API-Data\Test-Data.csv'
...
def fetch_weather_data(location, date):
    ...
    for i in range(retries):
        ...
        if response.status_code == 200:
            print(f"Successfully fetched data for {location} on {formatted_date}")
            return pd.read_csv(StringIO(response.text))
        elif response.status_code == 429:
            # Exponential backoff with a maximum of 32 seconds
            sleep_time = min(2 ** i, 32)
            print(f"Rate limit hit. Retrying in {sleep_time} seconds for {location} on {formatted_date}...")
...
for index, row in df.iterrows():
    ...
    print(f"Querying weather data for {location} on {date}")
